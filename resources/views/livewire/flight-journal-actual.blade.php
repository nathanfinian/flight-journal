<div>
    <x-ui.heading level="h1" size="xl" class="flex items-center justify-between">
        <div class="flex items-center">
           Actual Flight Hari Ini ({{ $this->hari }})
        </div>
        <div class="flex items-center">
            <x-ui.button 
                wire:navigate.hover
                ...
                variant="outline"
                icon="plus"
                :href="route('flight-journal.create')" 
            />
        </div>
    </x-ui.heading>
    ...
    <div class="flex gap-4">
        ...
        <x-ui.navbar.item 
            ...
            icon:class="w-5 h-5" 
            label="Actual" 
            :href="route('flight-journal.actual')"
            :active="request()->is('flight-journal*')"
        />
    </div>
   ...
   <table class="border border-collapse min-w-full text-sm rounded-xl overflow-hidden mt-4 shadow-lg">
        <thead class="bg-gray-200 dark:bg-neutral-800">
            <tr>
                ...
                <th class="px-4 py-3 text-left">Origin Equipment</th>
                <th class="px-4 py-3 text-left">Departure Equipment</th>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-neutral-800">
            @forelse($actualFlights as $index => $flight)
                <tr class="hover:bg-gray-50 dark:hover:bg-neutral-800/60 cursor-pointer" wire:click="openEdit({{ $flight->id }})">
                    ...
                    <td class="px-4 py-3">{{ $flight->originEquipment->registration ?? '—' }}</td>
                    <td class="px-4 py-3">{{ $flight->departureEquipment->registration ?? '—' }}</td>
